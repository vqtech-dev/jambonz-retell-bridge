...
if (internalExtension) {
  session.locals.logger.info(
    `${to} is an internal extension (${internalExtension}), dialing as SIP to SkySwitch domain`
  );

  target = [
    {
      type: 'sip',
      sipUri: `sip:${internalExtension}@${process.env.SKYSWITCH_EXTENSION_DOMAIN}`
    }
  ];
} else {
  target = [
...
/* Workaround for SIP identity formatting */
