...
// Registration-based SkySwitch carrier for inbound PSTN and outbound transfer legs.
// Separate from RETELL_TRUNK_NAME which connects to Retell only.
const SKYSWITCH_CARRIER_NAME = process.env.SKYSWITCH_CARRIER_NAME
  || process.env.PSTN_TRUNK_NAME
  || 'SkySwitch-JambonzRetell';
const SKYSWITCH_SIP_REALM = process.env.SKYSWITCH_SIP_REALM || 'visionquest.22393.service';
const SKYSWITCH_REGISTER_USERNAME = process.env.SKYSWITCH_REGISTER_USERNAME || 'KickCalltest';
const SKYSWITCH_EXTENSION_DOMAIN = process.env.SKYSWITCH_EXTENSION_DOMAIN || SKYSWITCH_SIP_REALM;
...
const logTransferSipResponse = (logger, context, evt) => {
  ...
  const payload = {
    ...
    carrier: SKYSWITCH_CARRIER_NAME,
    evt
  };
...
const buildSkySwitchTransferHeaders = (callerId, extraHeaders = {}) => {
  const headers = {...extraHeaders};
  const registeredIdentity = `<sip:${SKYSWITCH_REGISTER_USERNAME}@${SKYSWITCH_SIP_REALM}>`;

  // Match working KickCalltest trace: registered user in From/PAI, caller in display.
...
const service = ({logger, makeService}) => {
  ...
  svc.on('session:new', async(session) => {
    ...
    if (session.direction === 'inbound' &&
      SKYSWITCH_CARRIER_NAME && process.env.RETELL_SIP_CLIENT_USERNAME &&
      authUserHeader) {
    ...
    try {
      ...
      if (outboundFromRetell) {
        /* Warm transfer leg: originate through the registered SkySwitch carrier
           so jambonz reuses the KickCalltest registration (407/Proxy-Auth flow). */
        ...
        if (internalExtension) {
          session.locals.logger.info({
            ...
            internalExtension,
            carrier: SKYSWITCH_CARRIER_NAME,
            domain: SKYSWITCH_EXTENSION_DOMAIN
          }, 'Transfer target is on-net extension, dialing via registered carrier');

          // Route through the registration trunk — bare SIP URI without trunk
          // caused trunk=user and 403 in prior failed transfers.
          target = [
            {
              type: 'phone',
              number: internalExtension,
              trunk: SKYSWITCH_CARRIER_NAME
            }
          ];
        } else {
          session.locals.logger.info({
        ...
        headers = buildSkySwitchTransferHeaders(from, headers);
...
const onRefer = (session, evt) => {
  ...
  logger.info({
    refer_details,
    carrier: SKYSWITCH_CARRIER_NAME,
    register_username: SKYSWITCH_REGISTER_USERNAME,
    sip_realm: SKYSWITCH_SIP_REALM
